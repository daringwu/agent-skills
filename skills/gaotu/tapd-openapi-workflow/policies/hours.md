...
上游是活动清单：先由 `policies/shared/work-attribution.md` 的口径把 git/文档痕迹归纳成「做了哪些事」，
**这一层再把活动清单变成工时数字**。工时是 TAPD 的字段需求，所以估算口径属于这里，不属于共享层。

不要凭任务名反推工时。**估出来的数字必须让用户确认过再写入 TAPD** —— 工时会被用于考核，错误成本不对称。

### 待定义（TODO）

- **客观信号**：有没有可用的（commit 时间跨度 / 日报 / 会议日历），还是全靠估？
- **没有客观信号时**：我给一个估算让你改，还是直接问你？
- **分摊方式**：先定当天总数再往任务上分摊，还是逐任务估完再加总？见 `policy.json` 的 `hoursAllocation`。

## 边界情况（待定义）

- **跨天但不连续**（周一做一半、周四接着做）：日期怎么落？只落有实际推进的天，还是连续铺开？
- **同一天多个任务**：工时怎么分？平均分 / 按 commit 数 / 每次问你。见 `policy.json` 的 `sameDaySplit`。
- **被打断、等待、被阻塞的时间**：算不算？算的话记在哪个任务上？
